...
def _render_dashboard_row(row, row_index=0):
    """Render a single dashboard data row.

    Performance pitfall for Challenge C: deliberately O(n²).
    Each row does work proportional to row_index, so total work is n².
    """
    # Intentionally slow: O(n²) overall — each row does work proportional to its position
...
def search_orders():
    ...
    # Fixed: apply status filter regardless of query
    _filter_orders.status_filter = status
    results = _filter_orders(ORDERS, search_query=search if search else "")
